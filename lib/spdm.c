...
static void spdm_err(struct device *dev, struct spdm_error_rsp *rsp)
{
 ...
 *	Affect the data layout of messages.
...
struct spdm_state {
	...
	/*
	 * The base hash algorithm is not known until we reach the
...
static int spdm_append_buffer_a(struct spdm_state *spdm_state, const void *data,
			   ...
			   void *rsp, size_t rsp_sz)
{
	...
	if (response->code == SPDM_ERROR) {
		spdm_err(spdm_state->dev, (struct spdm_error_rsp *)rsp);
		return -EPROTO;
	}
...
{
	static const struct spdm_get_version_req req = {
		.version = 0x10,
...
retry:
	...
	rc = __spdm_exchange(spdm_state, &req, sizeof(req), rsp, rsp_sz);
	if (rc < 0)
		goto err;
	...
	if (length < struct_size(rsp, version_number_entries,
	...
	/*
	 * Cache the request and response to later compute a hash,
	 * the algorithm of which is not yet known.
	 *
	 * Response may contain fewer entries than numversions,
	 * so compute the length from the response's entry count.
	 */
...
static int spdm_get_capabilities(struct spdm_state *spdm_state)
{
	...
	rc = spdm_append_buffer_a(spdm_state, &req, req_sz, false);
	if (rc)
		return rc;

	return spdm_append_buffer_a(spdm_state, &rsp, rsp_sz, false);
}
			     ...
			     void *req, size_t req_sz, void *rsp, size_t rsp_sz)
{
	...
	case SPDM_HASH_SHA_384:
		spdm_state->shash = crypto_alloc_shash("sha384", 0, 0);
		break;
	case SPDM_HASH_SHA_256:
		spdm_state->shash = crypto_alloc_shash("sha256", 0, 0);
		break;
	default:
...
static int spdm_negotiate_algs(struct spdm_state *spdm_state)
{
	...
	/* Request length shall be <= 128 bytes (SPDM 1.1.0 Table 15) */
	...
	/* Responder shall select exactly 1 algorithm (SPDM 1.1.0 sec 10.4) */
...
{
	struct spdm_digest_req req = { .code = SPDM_GET_DIGESTS };
...
struct spdm_state *spdm_create(spdm_transport *transport, void *priv, u32 sz,
			       struct device *dev, struct key *keyring)
{
