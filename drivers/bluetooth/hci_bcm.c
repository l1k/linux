...
static int bcm_apple_get_resources(struct bcm_device *dev)
{
	...
	if (!adev ||
		...
		dev_err(&dev->pdev->dev, "Apple ACPI methods not found\n");
		return -ENODEV;
	}
	...
	if (!acpi_dev_get_property(adev, "baud", ACPI_TYPE_BUFFER, &obj) &&
		...
		dev_err(&dev->pdev->dev, "Apple baud property not found\n");

	dev->set_device_wakeup = bcm_apple_set_device_wakeup;
	dev->set_shutdown = bcm_apple_set_shutdown;

	dev_info(&dev->pdev->dev, "Apple resources found\n");
...
static int bcm_platform_probe(struct bcm_device *dev)
{
	...
	dev->device_wakeup = devm_gpiod_get(&pdev->dev, "device-wakeup",
					    GPIOD_OUT_LOW);
	if (IS_ERR(dev->device_wakeup))
		return PTR_ERR(dev->device_wakeup);

	dev->shutdown = devm_gpiod_get(&pdev->dev, "shutdown", GPIOD_OUT_LOW);
