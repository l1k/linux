...
{
	size_t length, payload_length, remainder;
	struct pci_dev *pdev = doe_mb->pdev;
	int offset = doe_mb->cap_offset;
	int ret, i = 0;
	u32 val;
...
void pci_doe_init(struct pci_dev *pdev)
{
	...
	while ((off = pci_find_next_ext_capability(pdev, off,
						   PCI_EXT_CAP_ID_DOE))) {
		doe_mb = pci_doe_create_mb(pdev, off);
		if (IS_ERR(doe_mb))
			continue;

		rc = xa_insert(&pdev->doe_mbs, off, doe_mb, GFP_KERNEL);
		if (rc) {
			pci_doe_flush_mb(doe_mb);
